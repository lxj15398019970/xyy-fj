...
function initUI(_box){
	...
	if ($.fn.xheditor) {
		$("textarea.editor", jParent).xheditor({
			skin: 'vista'
		});
	}
	if ($.fn.uploadify) {
	...
	$("input[type=text]", jParent).filter("[alt]").inputAlert();
	...
	$("a[target=dialog]", jParent).each(function(){
		$(this).click(function(event){
			...
			if(max) options.max = eval(max);
			if(mask) options.mask = eval(mask);
			var maxable = $this.attr("maxable");
			var minable = $this.attr("minable");
			if (maxable) options.maxable = eval(maxable);
			if(minable) options.minable = eval(minable);
			var resizable = $this.attr("resizable");
			var drawable = $this.attr("drawable");
			if (resizable) options.resizable = eval(resizable);
			if(drawable) options.drawable = eval(drawable);
