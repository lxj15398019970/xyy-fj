                ...
                success: function (response) {
                    ...
                    if (json.statusCode == DWZ.statusCode.error) {
                        ...
                        if (response.indexOf("loginPage") > 0) {
                            alertMsg.error("登录失效，请重新登录", {okCall: function () {
//                             DWZ.loadLogin();
                                window.location.href = DWZ._set.loginUrl;
                            }});
                        } else {
        ...
        return this.each(function () {
            ...
            $(op.toggleBut, bar).click(function () {
                ...
                $(op.container).animate({left: cleft, width: cwidth}, 50, function () {
                    bar.animate({left: barleft}, 500, function () {
                        bar.hide();
                        sbar.show().css("left", -50).animate({left: 0}, 200);
            ...
            $(op.toggleBut, sbar).click(function () {
                ...
                bar.animate({left: 0}, 800, function () {
                    ...
                    var cleft = 0 + bar.outerWidth() + $(op.splitBar).outerWidth();
                    var cwidth = $(op.container).outerWidth() - (cleft - $(op.container).cssv("left"));
//                    var cwidth = $(op.container).outerWidth();
    ...
    $.extend($.fn, {
        jTree: function (options) {
            ...
            return this.each(function () {
                ...
                //$('.tree li:first .treeTitle').addClass('selected');
                setTimeout(function () {
        ...
        genTree: function (options) {
            ...
            return this.each(function () {
                ...
                if (tree.size() > 0) {
                    node.children(":first").wrap("<div class='treeTitle oneList'></div>");
                    ...
                    node.children().wrap("<div class='oneList'></div>");
...
function initLayout() {
    ...
    var iContentH = $(window).height() - $("#header").height() - 60;
    $("#container").width(iContentW);
    $("#container .tabsPageContent").height(iContentH - 60).find("[layoutH]").layoutH();
...
function initUI(_box) {
    ...
    $("a[target=navTab]", $p).each(function () {
        $(this).click(function (event) {
            ...
            DWZ.debug(url);
            if (!url.isFinishedTm()) {
                alertMsg.info($this.attr("warn") || DWZ.msg("alertSelectMsg"));
                return false;
            }
...
var alertMsg = {
    ...
    _open: function (type, msg, buttons) {
        ...
        $(boxHtml).appendTo("body").css({top: -$(this._boxId).height() + "px"}).animate({top: "30px"}, 500);
        if (this._closeTimer) {
        ...
        tabs: function (options) {
            ...
            function initTab(jT) {
                ...
                jTabs.each(function (iTabIndex) {
                    ...
                    if (op.eventType == "hover")$(this).hover(function (event) {
                        switchTab(jT, iTabIndex)
                    });
                    else $(this).click(function (event) {
                        switchTab(jT, iTabIndex)
                    });
    ...
    $.extend({
        bringBackSuggest: function (args) {
            ...
            $box.find(":input").each(function () {
                ...
                //var $input = $(this), inputName = $input.attr("name");//改为由ID识别赋值
                for (var key in args) {
...
(function ($) {
    $.extend($.fn, {
        jBlindUp: function (options) {
            var op = $.extend({duration: 500, easing: "swing", call: function () {
            }}, options);
            return this.each(function () {
                var $this = $(this);
        ...
        jBlindDown: function (options) {
            var op = $.extend({to: 0, duration: 500, easing: "swing", call: function () {
            }}, options);
            return this.each(function () {
                var $this = $(this);
